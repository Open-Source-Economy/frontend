    ...
    return (
        <div className=" relative p-[0] pr-6 ssm:p-[1rem] sm:p-[2rem] flex-col lg:flex-row  flex gap-4 justify-between ">
            <div className='overflow-hidden pt-lg-5 mt-lg-5' style={{ width: "100%", height: "500px" }}>
                ...
                <div className="tabs ">
                    <button className={active == "tab-1" ? "active" : "tab"} onClick={() => { setActive("tab-1"); setClick("24H") }}>
                        24h
                    </button>
                    <button className={active == "tab-2" ? "active" : "tab"} onClick={() => { setActive("tab-2");setClick("1W") }}>
                        1W
                    </button>
                    <button className={active == "tab-3" ? "active" : "tab"} onClick={() => { setActive("tab-3");setClick("1M") }}>
                        1M
                    </button>
                    <button className={active == "tab-4" ? "active" : "tab"} onClick={() => { setActive("tab-4"); setClick("1Y") }}>
                        1Y
                    </button>
                    <button className={active == "tab-5" ? "active" : "tab"} onClick={() => { setActive("tab-5"); setClick("All") }}>
                        All
                    </button>
                </div>
                ...
                <ResponsiveContainer width="100%" className="transformscale" height="67%">
                    ...
                    >
                        <XAxis interval={0} axisLine={false} xAxisId="0" stroke="#9fa5ae" dataKey="days" className="font-normal font-mono text-[8px] ssm:text-[10px] xsm:-[14px] sm:text-[16px]" />
                        <YAxis axisLine={false} includeHidden domain={[minnPrice, maxxPrice]} stroke="#9fa5ae" className="  font-normal no-underline font-mono text-[8px] ssm:text-[10px] xsm:-[14px] sm:text-[16px]" />
                        ...
                        <Tooltip contentStyle={{ background: "transparent", fontFamily: "monospace", fontSize: "16px", fontWeight: "300", borderRadius: "10px" }} labelStyle={{ display: "none", color: "#222" }} />
                    </AreaChart>
                </ResponsiveContainer>
