...
export function UserCondition(props: ProblemProps) {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      ...
      { opacity: 0, x: -50 },
      { opacity: 1, x: 0, duration: 1, stagger: 0.1, ease: "power2.out", delay: 1.2 }
    );
    gsap.fromTo(
      textBlockRefs2.current,
      { opacity: 0, x: 50 },
      { opacity: 1, x: 0, duration: 1, stagger: 0.1, ease: "power2.out", delay: 1.2 }
    );
    ...
    <>
      ...
      <div className="w-full absolute z-[-10] pointer-events-none flex justify-between mt-14">
          <div className="text-[#ffffff1a] leading-[204.348%] font-michroma max-w-[653px] w-full ">
            ...
            <p ref={(el) => (textBlockRefs.current[1] = el!)} className="3xl:text-[28px] lg:text-xl md:text-lg sm:text-base text-sm mx:mt-[170px] mt-20 text-start max-w-[650px] mx-auto">
              Will my issues be fixed?
            </p>
            <p ref={(el) => (textBlockRefs.current[2] = el!)} className="3xl:text-[23px] lg:text-lg md:text-base xs:text-sm text-xs 3xl:mt-[100px] mx:mt-20 sm:mt-16 mt-10 mb-[57px] xl:max-w-[471px] xl:text-center text-start w-full mx-auto">
              No eyes on my PR!
            </p>
            <p ref={(el) => (textBlockRefs.current[3] = el!)} className="3xl:text-[23px] lg:text-lg md:text-base xs:text-sm text-xs md:mt-36 sm:mt-24 mt-16 max-w-[639px] text-start mx:ml-24 xs:ml-6 w-full">
              No influence <br /> over governance
            </p>
          </div>
          <div className="text-[#ffffff1a] leading-[204.348%] font-michroma w-full max-w-[636px]">
            <p ref={(el) => (textBlockRefs2.current[0] = el!)} className="3xl:text-[23px] lg:text-lg md:text-base xs:text-sm text-xs text-center max-w-[540px] w-full mx:ml-24 xs:ml-16">
              No customer support
            </p>
            <p ref={(el) => (textBlockRefs2.current[1] = el!)} className="3xl:text-[31px] md:text-xl sm:text-lg xs:text-base text-sm mb-[60px] 3xl:mt-[350px] 2xl:mt-[300px] sm:mt-36 mt-20 xl:max-w-[391px] xl:text-center text-end w-full mx-auto">
              Security concerns?
            </p>
            <p ref={(el) => (textBlockRefs2.current[2] = el!)} className="3xl:text-[31px] md:text-xl sm:text-lg xs:text-base text-sm xl:mt-[150px] mx:mt-20 mt-10 xl:text-start text-end xl:max-w-[471px] w-full ">
        ...
        <div className="flex items-center justify-between mx:gap-8 sm:gap-4 gap-3 3xl:max-w-[1290px] max-w-[980px] w-full mx-auto relative 3xl:pt-14 pt-20">
          <div
            ...
            className="mx:max-w-[319px] w-fit lg:space-y-[194px] mx:space-y-32 space-y-20 h-full relative z-10"
          >
          ...
          <img
            ref={mainBgRef}
            className="absolute -z-10 w-full h-[200%] -top-[30%] object-center"
            src={props.mainBg}
            alt="mainBg"
          />
          <img
          ...
          <div
            ...
            className="flex items-center justify-center xl:text-[23px] xl:text-lg lg:text-base mx:text-sm xs:text-xs text-[10px] font-michroma mx:max-w-[319px] relative z-10 bg-[#FF518C] lg:!p-[15px] xs:p-2 p-1 rounded-[10px] max-w-[319px] w-fit"
          >
            {props.subtittle1}
          </div>
        </div>
