...
interface SelectProjectsModalProps {
  service: dto.Service;
  developerService: dto.DeveloperService;
  availableProjects: [dto.ProjectItem, dto.DeveloperProjectItem][];
  onClose: () => void;
  onUpdateTask: (updatedDevService: dto.DeveloperService) => void;
  onBack: () => void;
  isLoading: boolean;
}
...
export default function SelectProjectsModal(props: SelectProjectsModalProps) {
  const { service, developerService, availableProjects, onClose, onUpdateTask, onBack, isLoading } = props;

  const [selectedProjectId, setSelectedProjectId] = useState<string>(developerService.projectItemId?.uuid || "");
  const [hourlyRate, setHourlyRate] = useState<string>(developerService.hourlyRate?.toString() || "100");
  const [currency, setCurrency] = useState<dto.Currency>(/*developerService.currency || */ dto.Currency.EUR); // TODO: lolo
  const [customResponseTime, setCustomResponseTime] = useState<string>(developerService.responseTimeHours?.toString() || "12");

  const handleSave = () => {
    const updatedDevService = {
      ...developerService,
      projectItemId: new dto.ProjectItemId(selectedProjectId),
      hourlyRate: Number(hourlyRate),
      currency: currency,
      responseTimeHours: service.hasResponseTime ? Number(customResponseTime) : null,
    };
    onUpdateTask(updatedDevService);
  };
        ...
        <div className="mb-4">
          ...
          <div className="bg-[#202f45] rounded-md p-3">
            {isLoading ? (
              ...
              availableProjects.map(([projectItem, devProjectItem]) => {
                const isSelected = selectedProjectId === devProjectItem.id.uuid;
                return (
                    ...
                    <label className="flex flex-row gap-2 items-center cursor-pointer">
                      <div className="bg-[#202f45] relative rounded-sm shrink-0 size-[18px]">
                        <input
                          ...
                          checked={isSelected}
                          onChange={() => setSelectedProjectId(devProjectItem.id.uuid)}
