...
import { CommentIcon, CommentSection, ProjectSelection, ResponseTimeSection } from "./components";
...
export function EditServiceModal(props: EditServiceModalProps) {
  ...
  useEffect(() => {
    if (props.isOpen) {
      setSelectedDeveloperProjectItemIds(props.developerServiceEntry.developerService?.developerProjectItemIds || []);
      ...
      <ModalBackdrop isOpen={props.isOpen} onClose={props.onClose} className="relative w-full max-w-[800px] mx-4 bg-[#0E1F35] rounded-[50px] p-9">
        ...
        <div className="flex flex-col items-end gap-6 self-stretch p-9 rounded-[30px] bg-[#14233A] mb-8">
          <ProjectSelection
            ...
            selectedProjectItemIds={selectedDeveloperProjectItemIds}
        ...
        <div className="flex justify-end items-center gap-6 self-stretch mt-8">
          ...
          <button
            onClick={handleSave}
            disabled={selectedDeveloperProjectItemIds.length === 0 || isLoading}
            className={`flex justify-center items-center gap-2 px-5 py-3 rounded-md border border-[#FF7E4B] transition-opacity ${
              selectedDeveloperProjectItemIds.length === 0 || isLoading ? "opacity-50" : "opacity-100 hover:opacity-70"
            }`}
          >
