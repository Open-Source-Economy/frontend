...
//   // Corrected type to match what SelectProjectsModal expects for developerService
...
//       }
//
//       // Create a new DeveloperServiceTODOChangeName with default values
//       const newDevService: DeveloperServiceTODOChangeName = {
//         // ID is usually generated by the backend on creation, so we don't set it here for new ones
//         serviceId: service.id,
//         projectItemIds: [], // Start with no projects selected
//         hourlyRate: 100, // Sensible default
//         responseTimeHours: service.hasResponseTime ? 12 : undefined, // Default if applicable
//         // currency, createdAt, updatedAt are typically added by the backend or in a later step
//       };
//       return [service, newDevService] as [dto.Service, DeveloperServiceTODOChangeName];
//     }).filter(s => s !== null) as [dto.Service, DeveloperServiceTODOChangeName][];
//
//     const updatedServices: [ServiceId, DeveloperServiceTODOChangeName | null][] = [...state.services, ...newServices];
//     updateState({ services: updatedServices });
//     setShowTaskSelectionModal(false);
//   };
//
//   const handleRemoveTask = (devServiceId: string) => {
//     const updatedServices = state.services.filter(s => s[1].id?.uuid !== devServiceId); // Use optional chaining for id
//     updateState({ services: updatedServices });
//   };
//
...
//       if (s[1].id?.uuid === updatedDevService.id?.uuid) { // Use optional chaining for id
...
//         // Prepare the body for upsert
//         const body: dto.UpsertDeveloperServiceBody = {
//           developerService: {
//             serviceId: service.id,
//             projectItemIds: devService.projectItemIds, // Correctly use array
//             hourlyRate: devService.hourlyRate,
//             responseTimeHours: devService.responseTimeHours || undefined,
//             currency: devService.currency || currency, // Ensure currency is included
//           },
//         };
//         // If it's an existing service, include its ID for update
//         if (devService.id?.uuid) {
//           body.developerService.id = devService.id;
//         }
//
...
//       return acc;
//     },
//     [] as { category: string; tasks: [dto.Service, DeveloperServiceTODOChangeName][] }[],
//   );
//
//   return (
...
//                       // Find the corresponding ProjectItem from projectItems state
...
//                   return (
//                     <div
//                       key={devService.id?.uuid || service.id.uuid} // Use devService.id for key, fallback to service.id
...
//                           </button>
//                           <button
//                             onClick={() => handleRemoveTask(devService.id?.uuid || service.id.uuid)} // Use devService.id, fallback to service.id
