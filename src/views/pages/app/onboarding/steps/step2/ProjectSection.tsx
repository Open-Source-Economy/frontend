...
export function ProjectSection(props: ProjectSectionProps) {
  const availableRepositories = props.selectedOrganization
    ? mockRepositories[props.selectedOrganization as keyof typeof mockRepositories] || []
    : [];
  ...
  const renderProjectTypeInputs = () => {
    switch (props.selectedProjectType) {
      ...
      default:
        return (
          <div className="text-white font-montserrat text-base opacity-60">
            Please select a project type above
          </div>
        );
    }
  };
    ...
    <div className="flex p-9 flex-col justify-end items-end gap-2.5 self-stretch rounded-[30px] bg-[#14233A]">
      ...
      <div className="flex flex-col items-start gap-2 self-stretch">
        <ProjectItemTypeSelectInput
          value={props.selectedProjectType}
          onChange={props.onProjectTypeChange}
          required
          ref={props.projectTypeSelectRef}
        />
      </div>
