...
export function UserProfileSection(props: UserProfileSectionProps) {
  ...
  const name = AuthenticatedUserCompanion.getName(props.user) || "";
  const email = AuthenticatedUserCompanion.getEmailOrGithubLogin(props.user) || "";
    ...
    return (
      <div className="bg-gradient-to-r from-brand-primary to-brand-primary/90 px-3 py-3 mb-[4px] rounded-t-md mt-[-4px] mr-[-4px] ml-[-4px]">
        <div className="flex items-center gap-3">
          <UserAvatar user={props.user} className="h-10 w-10 border-2 border-white/20" />
  ...
  return (
    <div className="px-2 py-2 border-b border-brand-neutral-300">
      <div className="flex items-center gap-3">
        <UserAvatar user={props.user} className="h-10 w-10 border-2 border-brand-accent" />
        <div className="flex-1 min-w-0">
