  ...
  return (
    <div className="flex flex-col items-center gap-6 self-stretch py-6 rounded-md">
      ...
      <div className="flex justify-center items-center gap-2.5 self-stretch">
        <div className="flex-1 text-white font-michroma text-[28px] font-normal leading-[1.3]">The Service Model</div>
        <button
          onClick={onClose}
      ...
      <div className="flex justify-center items-start gap-3 self-stretch">
        {steps.map((step, index) => (
          <ServiceModelStep key={index} stepNumber={step.stepNumber} description={step.description} />
        ))}
      </div>
    </div>
  );
